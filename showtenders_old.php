...
<?php
	...
	if(isset($keyword , $location , $cpv , $sector ,$deadline_frm, $deadline_to, 
	...
	}  else if(	(   !empty($keyword) ||  !empty($cpv) || !empty($sector)) &&
				(   !empty($location) || !empty($deadline_frm) || !empty($deadline_to) || 
					!empty($funding_type) || !empty($financier) ||
					!empty($live) || !empty($tender_type) || !empty($tender_value )) )
		{
				...
				if (!empty($live_table) && empty($keyword) && empty($location) && empty($cpv) && empty($sector) && empty($deadline_frm) && empty($deadline_to) && empty($funding_type ) && empty($financier) && empty($tender_type) && empty($tender_value )     )
					{
						$query = "SELECT * FROM `".$live_table."` ";
					}
				...
				###
						...
						$pagination .= "<a href='showtenders.php?q=advancedsearch&Page=allAds";
									if(!is_null($keyword)){
						...
						$pagination .= "<li $active><a href='showtenders.php?q=advancedsearch&Page=allAds";
									if(!is_null($keyword)){
										$pagination .= "&ikeyword=" .$keyword;
									}
									if(!is_null($location)){
						...
						$pagination .=  "<li >
											<a class='disabled' href='showtenders.php?q=advancedsearch&Page=allAds";
											if(!is_null($keyword)){
										$pagination .= "&ikeyword=" .$keyword;
									}
									if(!is_null($location)){
					...
					isset($_GET['sector']) || isset($_GET['financier']) || isset($_POST['index']) )	
					{
						...
						#session_start();			
						@$_SESSION['url'] = $Exploded_URL[4]; # Exploded_URL[3] for deployment  |  Exploded_URL[4] for localhost
						# echo $urlToCheck;
