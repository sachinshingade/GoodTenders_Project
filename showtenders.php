	...
	<section class='inner-header-title'>
			<div class='container'>
				...
				<div class="row">
					...
					<button type="reset" class="btn btn-link" style="color:#07b107;text-decoration:underline;float: right;" id="adreset" onclick="resetSelect()">Reset</button>
...
<?php
	...
	if(isset($keyword ) || isset($location) || isset($cpv) || isset($sector) || isset($deadline_frm) || isset($deadline_to) ||
			isset($funding_type) || isset($financier) || isset($live) || isset($tender_type) || isset($tender_value) )
			{
				...
				//echo $query;
				//echo '<br>===query===<br>';
				$pquery = substr($query, 0 , -18);
				$pquery = $pquery." LIMIT 1000";

				$pwl = substr($query, 0 , -12);
				$pwl = $pwl." LIMIT 100";
				...
				//Pagination on search and advance search when user not logged in
					if(empty($_SESSION["id"]))
					{	
						$pwlresult = mysqli_query($conn,$pwl);
						$total_records = mysqli_num_rows($pwlresult);  //count number of records		
						if($total_records > 10 && $total_records <= 20){
							$total_pages = 2;	
	...
	if(isset($keyword ) || isset($location) || isset($cpv) || isset($sector) || isset($deadline_frm) || isset($deadline_to) ||
			isset($funding_type) || isset($financier) || isset($live) || isset($tender_type) || isset($tender_value) )
			{
				...
				###
						...
						//echo $ptotal_records." ".$ptotal_pages;
						if ($ptotal_pages <= 10) {
						    $start = 1;   // Startin pagination
						    $end   = $ptotal_pages;   // End at total pages
						} else { 
	...
	} else if(@$urlToCheck == 'tenderbycountry.php')
			{
				...
				if(empty($_SESSION["id"]))
					{
						...
						$presult = mysqli_query($conn,$psql);
						$ptotal_records = mysqli_num_rows($presult);  //count number of records						
						if($ptotal_records > 10 && $ptotal_records <= 20){
							$ptotal_pages = 2;	
						}else if($ptotal_records > 20){
							$ptotal_pages = 3;
						}else{				
							$ptotal_pages = 1;
						}
				...
				###
						...
						$pagination .=  "<li >
											<a class='disabled' href='showtenders.php?scountry=" . $country . "&Page=allAds&page=$ptotal_pages'>" . 
											'&raquo;' . "</a> </li>
										</ul></div>";
						#echo $pagination;
					}else if(isset($_SESSION["id"])){
						...
						$psql = 'SELECT B.*, A.Country FROM tbl_tenders AS B 
							INNER JOIN tbl_country as A 
							ON (B.org_country=A.code) 
							WHERE A.Country = "'.$country.'" LIMIT 500';
	 ...
	 }  else if(@$urlToCheck == 'tenderbyregion.php')
			{
				...
				###
						...
						$pagination .=  "<li >
											...
											$region . "&Page=allAds&page=$ptotal_pages'>" . 
											'&raquo;' . "</a> </li>
										</ul></div>";
						#echo $pagination;
					}else if(isset($_SESSION["id"])){
				...
				$sql = 'SELECT B.*, A.Country,C.Country_Short_Code FROM tbl_tenders AS 
					...
					if($ptotal_records > 10 && $ptotal_records <= 20){
						...
						$pagination .=  "<li >
											...
											$data . "&Page=allAds&page=$ptotal_pages'>" . 
											'&raquo;' . "</a> </li>
										</ul></div>";
						#echo $pagination;
					}else if(isset($_SESSION["id"])){
				...
				$sql = 'SELECT B.*, A.Country,C.id FROM tbl_tenders 
					...
					if($ptotal_records > 10 && $ptotal_records <= 20){
						...
						$pagination .=  "<li >
											...
											$financier . "&Page=allAds&page=$ptotal_pages'>" . 
											'&raquo;' . "</a> </li>
										</ul></div>";
						#echo $pagination;
					}else if(isset($_SESSION["id"])){
			...
			if(empty($_SESSION["id"]))
			{
				...
				$ptotal_records = mysqli_num_rows($presult);  //count number of records						
					if($ptotal_records > 10 && $ptotal_records <= 20){
						...
						$pagination .=  "<li >
											<a class='disabled' href='showtenders.php?&Page=allAds&page=$ptotal_pages'>" . 
											'&raquo;' . "</a> </li>
										</ul></div>";
						#echo $pagination;
			}  else if(isset($_SESSION["id"])) {
