...
<style type="text/css">
	...
	<section>
        ...
        if(empty($_SESSION["id"]))
		{
            ...
            ON B.sector = C.Sub_Sector_Id ORDER BY B.id LIMIT $start_from, $num_rec_per_page";		

  			$result = mysqli_query($conn,$sql);
			$row = mysqli_num_rows($result);
  			$total_records = mysqli_num_rows($result);  

  			$psql = "SELECT B.*, A.Country,C.Sector_Name FROM tbl_news AS B 
			INNER JOIN tbl_country as A 
			ON (B.country=A.code)
			LEFT JOIN tbl_sector C
            ON B.sector = C.Sub_Sector_Id ORDER BY B.id";		
			...
			$prow = mysqli_num_rows($presult);
  			...
  			global $pagination;
						...
						$pagination .= "<li $active><a href='news.php?Page=allAds&page=" . $i . "'>" . $i . "</a></li>";
						}
						$pagination .=  "<li >
											<a class='disabled'>" . 
											'&raquo;' . "</a> </li>
			...
			if ($ptotal_pages <= 10) {
				...
				global $pagination,$start_from,$num_rec_per_page;
						...
						$pagination .=  "<a href='news.php?Page=allAds&page=1'>" . '&laquo;' . "</a> </li>"; 
						// Goto 1st page  
						...
						$pagination .= "<li $active><a href='news.php?Page=allAds&page=" . $i . "'>" 
											. $i . 
											"</a></li>";
						}
						$pagination .=  "<li >
											<a class='disabled' href='news.php?Page=allAds&page=$total_pages'>" 
											. '&raquo;' . "</a> 
										</li>
										</ul></div>";
			...
			<div class='advance-search-job-locat'>
				<h5><img src='assets/flags/blank.gif' class='flag flag-".strtolower($row['country'])."'/> ".$row['Country']."</h5>
			</div>
			</div>
			<div class='col-md-3 col-sm-3'>
			<div class='advance-search-job-locat'>";
		...
		?>  
            ...
            <div class="modal-header" style="background-color: #07b107;color: #ffffff;">
              ...
              <h3 class="modal-title text-center">Business News</h3>
            </div>
            ...
            <?php
                ...
                echo "<div class='row'>
                    ...
                    <div class='col-md-12 tender_content'>
                    <h5><strong>Title:</strong> ".$row['headline']."</h5>
                    <hr>
                	</div>
                    <div class='col-md-12 tender_content'>
                    <h5><strong>Country:</strong>
                    <img src='assets/flags/blank.gif' class='flag flag-".strtolower($row['country'])."'/>
                     ".$row['Country']."</h5>     
                    </div>
                    ...
                    <div class='col-md-12 tender_content'>
                    <h5><strong>Description:</strong><br> ".strip_tags(substr($row['details'],0,200)).".....</h5>
                    </div>
                    </div>
                </div>";
                if(isset($_SESSION["id"])):
...
?>
	...
	<script>
		...
		function onRedirect(id){
				var noRedirect = '.btn';
  					window.open("newsdetails.php?nsid="+id,"_blank");
  				}
